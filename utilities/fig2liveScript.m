...
%     numeric array defining the wdth and the height of the figure in
%     pixels. If a scalar is supplied, it acts both as height and width
%     (default = []).
...
if ~isempty(options.figSize) % Adjust figure size
  ...
  if isscalar(options.figSize)
    options.figSize = [options.figSize options.figSize];
  end
  set(figH, 'Position',[position(1),position(2), ...
    options.figSize(1),options.figSize(2)])
end
