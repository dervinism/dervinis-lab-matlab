function timeSeries = lowpassFilterTimeSeries(timeSeries, options)
% filtTimeSeries = lowpassFilterTimeSeries(timeSeries, <options>)
%
% Low-pass filter a row matrix containing time series vectors.
%
% Args:
...
%
% Returns:
%   filtTimeSeries (numeric): a shape-(N, M) numeric array of low-pass
...
Wn_theta = options.frequencyCutoff(1)/(options.sampleRate/2); % Low-pass frequency normalised by the Nyquist frequency
